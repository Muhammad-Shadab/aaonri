...
class EventViewModel @Inject constructor(private val eventRepository: EventRepository) :
    ...
    fun getMyEvent(allEventRequest: AllEventRequest) = viewModelScope.launch {
        ...
        eventRepository.getAllEvent(allEventRequest).onSuccess {
            myEvent.postValue(Resource.Success(it))
        }
            .onFailure {
                myEvent.postValue(Resource.Error(it.localizedMessage))
            }
    ...
    fun getRecentEvent(userEmail: String) = viewModelScope.launch {
        ...
        eventRepository.getRecentEvent(userEmail).onSuccess {
            recentEventData.postValue(Resource.Success(it))
        }
            .onFailure {
                recentEventData.postValue(Resource.Error(it.localizedMessage))
            }
    ...
    fun getAllEvent(allEventRequest: AllEventRequest) = viewModelScope.launch {
        ...
        eventRepository.getAllEvent(allEventRequest).onSuccess {
            allEventData.postValue(Resource.Success(it))
        }
            .onFailure {
                allEventData.postValue(Resource.Error(it.localizedMessage))
            }
        /*val response = eventRepository.getAllEvent(allEventRequest)
