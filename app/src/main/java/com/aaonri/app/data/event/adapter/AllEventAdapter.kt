...
class HomeEventAdapter(private var selectedServices: ((value: UserEvent) -> Unit)) :
    RecyclerView.Adapter<HomeEventAdapter.EventViewHolder>() {

    private var data = listOf<UserEvent>()
    private var startDate: String? = null
    private var startTimeOfEvent: String? = null
    private var endTimeOfEvent: String? = null
    private var timeZone: String? = null
    ...
    @JvmName("setData1")
    fun setData(data: List<UserEvent>?) {
        if (data != null) {
            this.data = data
        }
        notifyDataSetChanged()
    ...
    inner class EventViewHolder(val binding: HomeEventItemBinding) :
        RecyclerView.ViewHolder(
            binding.root
        )
}